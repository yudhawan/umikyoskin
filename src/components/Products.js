...
function Products({id,product_name,stock,price,category,sub,images,description,grosir_min,grosir_price}) {
  ...
  return (
    <div key={id} className='shadow-lg my-2 mx-2 rounded-md flex flex-col space-y-1 w-36 lg:w-44 h-auto'>
        <div className=' w-36 h-36 lg:w-[11rem] lg:h-[11rem] flex justify-center items-center'>
            <img src={host+'/'+img} alt={product_name} className='w-full h-full' />
        </div>
        <div className='text-left p-1'>
            <div className='text-md font-medium line-clamp-1' alt={product_name}>{product_name}</div>
        ...
        <div className='flex w-full'>
          ...
          <div className='p-1 justify-end flex space-x-1'>
            <div className='bg-green-700 p-1 border rounded-md text-white font-semibold text-sm lg:text-md cursor-pointer'>Buy</div>
